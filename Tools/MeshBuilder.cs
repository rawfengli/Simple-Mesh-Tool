...
public static class MeshBuilder
{
    ...
    public static SimpleMesh SimpleMeshPretreatmentSQ(SimpleMesh simpleMesh)
    {
        ...
        dx = simpleMesh.length * 2 / (simpleMesh.row - 1);
        dy = simpleMesh.width * 2 / (simpleMesh.column - 1);
        for (int i = 0; i < simpleMesh.column; i++)//uv(0,0)
        {
            //move to i column
            float v = i * (1.0f / (float)(simpleMesh.column - 1));

            nextPosition = position + new Vector3(-simpleMesh.length, -simpleMesh.width, 0);
            nextPosition = nextPosition + new Vector3(0, i * dy, 0);
            for (int j = 0; j < simpleMesh.row; j++)
            {
                simpleMesh.vertices.Add(nextPosition);
                nextPosition = nextPosition + new Vector3(dx, 0, 0);

                float u = j * (1.0f / (float)(simpleMesh.row - 1));
                simpleMesh.uv.Add(new Vector2(u, v));
            }
        }

        for (int i = 0; i < simpleMesh.column - 1; i++)
        {
            for (int j = 0; j < simpleMesh.row - 1; j++)
            {
                simpleMesh.TriangleIndexs.Add(i * simpleMesh.row + j);
                simpleMesh.TriangleIndexs.Add((i + 1) * simpleMesh.row + j);
                simpleMesh.TriangleIndexs.Add(i * simpleMesh.row + j + 1);

                simpleMesh.TriangleIndexs.Add((i + 1) * simpleMesh.row + j + 1);
                simpleMesh.TriangleIndexs.Add(i * simpleMesh.row + j + 1);
                simpleMesh.TriangleIndexs.Add((i + 1) * simpleMesh.row + j);
            }
        }
    ...
    public static SimpleMesh SimpleMeshPretreatmentPL(SimpleMesh simpleMesh)
    {
        ...
        dx = simpleMesh.length * 2 / (simpleMesh.row - 1);
        dz = simpleMesh.width * 2 / (simpleMesh.column - 1);
        for (int i = 0; i < simpleMesh.column; i++)
        {
            //move to i column
            float v = i * (1.0f / (float)(simpleMesh.column - 1));
